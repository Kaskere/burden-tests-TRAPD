...
if (options.vcffilename.endswith(".gz") is False) and (
        options.vcffilename.endswith(".bgz") is False):  # if vcf filename is not given
...
def checkfilter(infofilter):
    if ("[" not in infofilter) or (infofilter.startswith("]")) or (infofilter.endswith("]")) or str(
            infofilter.split("[")[1].split("]")[0]) not in ["<", ">", "<=", ">=", "=", "!=", "in", "%"]:
        return 0
    else:
        return 1
...
for line_vcf1 in vcffile:
    ...
    if line_vcf[0][0] != "#":
        ...
        if options.bedfilename is not None:
            ...
            if chr not in bed_lower.keys():
                if chr not in skipped_chr:
                    sys.stdout.write("Failed to find chr in bed file, chr skipped: " + line_vcf[0] + "\n")
                    skipped_chr.insert(len(skipped_chr), chr)
                continue
            else:
